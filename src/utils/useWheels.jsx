...
export const useWheels = (width, height, front, radius) => {
    ...
    const wheelPosition = height * 0.3 // 바퀴 위치 계산. 차체 높이의 n배로 계산됨.
    ...
    const wheelInfos = [ // 차체와의 연결점과 바퀴가 앞바퀴인지 여부를 정의
        {
          ...wheelInfo,
    ...
    const wheelFunc = () => ({ // 바퀴에 적용될 물리적 속성을 정의
        collisionFilterGroup: 0,
        mass: 50,
        type : 'Kinematic',
        shapes: [
            {
            args: [wheelInfo.radius, wheelInfo.radius, 0.025, 16], // 원통의 반지름, 높이, 세그먼트 수 _ 세그먼트 수를 늘리면 보이는 부분이 더 매끄러워 짐
            rotation: [0, 0, -Math.PI / 2], // 회전
            type: "Cylinder",
            },
        ],
    });

    useCompoundBody(wheelFunc, wheels[0]); // 바퀴에 대한 물리적 바디를 생성
    useCompoundBody(wheelFunc, wheels[1]);
    useCompoundBody(wheelFunc, wheels[2]);
    useCompoundBody(wheelFunc, wheels[3]);

    return [wheels, wheelInfos] // 각 바퀴에 대한 참조와 각 바퀴의 정보를 반환
}